...
public class PostgresqlServiceCreatorTest {
	...
	public void cloudPostgreSQLDataSourceCreation() throws Exception {
		when(mockRuntime.getServiceInfos(PostgresqlServiceInfo.class))
			.thenReturn(Collections.singletonList(mockPostgresqlServiceInfo));
		...
		PostgresqlServiceCreator postgresqlCreationHelper = new PostgresqlServiceCreator(mockRuntime);
		DataSource dataSource = postgresqlCreationHelper.createSingletonService().service;
